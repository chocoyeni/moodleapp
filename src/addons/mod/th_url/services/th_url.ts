...
import { ADDON_MOD_TH_URL_COMPONENT_LEGACY } from '../constants';
...
export class AddonModThUrlProvider {

    protected static readonly ROOT_CACHE_KEY = 'mmaModThUrl:';
     ...
     */
    getFinalDisplayType(url?: AddonModThUrlUrl): ModResourceDisplay {
        if (!url) {
            return -1;
        }
    ...
    protected getUrlCacheKey(courseId: number): string {
        return `${AddonModThUrlProvider.ROOT_CACHE_KEY}url:${courseId}`;
    }
     ...
     */
    async getUrl(courseId: number, cmId: number, options: CoreSitesCommonWSOptions = {}): Promise<AddonModThUrlUrl> {
        const site = await CoreSites.getSite(options.siteId);

        const params: AddonModThUrlGetUrlsByCoursesWSParams = {
            courseids: [courseId],
        };
        ...
        const response = await site.read<AddonModThUrlGetUrlsByCoursesResult>('mod_thurl_get_urls_by_courses', params, preSets);
    ...
    logView(id: number, siteId?: string): Promise<void> {
        const params: AddonModThUrlViewUrlWSParams = {
            urlid: id,
        };
        ...
        return CoreCourseLogHelper.log(
            'mod_th_url_view_url',
            params,
            ADDON_MOD_TH_URL_COMPONENT_LEGACY,
            id,
            siteId,
        );
    }
...
export const AddonModThUrl = makeSingleton(AddonModThUrlProvider);

/**
 * Params of mod_th_url_get_urls_by_courses WS.
 */
type AddonModThUrlGetUrlsByCoursesWSParams = {
    courseids?: number[]; // Array of course ids.
};

/**
 * Params of mod_th_url_view_url WS.
 */
type AddonModThUrlViewUrlWSParams = {
    urlid: number; // Url instance id.
};

/**
 * URL returnd by mod_th_url_get_urls_by_courses.
 */
export type AddonModThUrlUrl = CoreCourseModuleStandardElements & {
...
/**
 * Result of WS mod_th_url_get_urls_by_courses.
 */
export type AddonModThUrlGetUrlsByCoursesResult = {
    urls: AddonModThUrlUrl[];
 ...
 */
export type AddonModThUrlDisplayOptions = {
    printintro?: boolean;
};
