...
import { AddonModThUrl } from '../th_url';
import { AddonModThUrlHelper } from '../th_url-helper';
import { CoreAnalytics, CoreAnalyticsEventType } from '@services/analytics';
import { CoreUrl } from '@singletons/url';
import { CoreMimetype } from '@singletons/mimetype';
import { ADDON_MOD_TH_URL_COMPONENT, ADDON_MOD_TH_URL_MODNAME, ADDON_MOD_TH_URL_PAGE_NAME } from '../../constants';
...
export class AddonModThUrlModuleHandlerService extends CoreModuleHandlerBase implements CoreCourseModuleHandler {

    name = ADDON_MOD_TH_URL_COMPONENT;
    modName = ADDON_MOD_TH_URL_MODNAME;
    protected pageName = ADDON_MOD_TH_URL_PAGE_NAME;
    ...
    async getIconSrc(module?: CoreCourseModuleData, modIcon?: string): Promise<string | undefined> {
        ...
        if (image.startsWith('f/')) {
            ...
            icon = mainFile ? AddonModThUrl.guessIcon(mainFile.fileurl) : undefined;
        }
    ...
    async getMainComponent(): Promise<Type<unknown>> {
        const { AddonModThUrlIndexComponent } = await import('../../components/index');

        return AddonModThUrlIndexComponent;
    }
    ...
    protected async shouldOpenLink(module: CoreCourseModuleData): Promise<boolean> {
        try {
            ...
            if (canHandle) {
                ...
                const url = await CorePromiseUtils.ignoreErrors(AddonModThUrl.getUrl(module.course, module.id));
                const displayType = AddonModThUrl.getFinalDisplayType(url);
    ...
    protected async logView(module: CoreCourseModuleData): Promise<void> {
        try {
            if (module.instance) {
                await AddonModThUrl.logView(module.instance);
...
export const AddonModThUrlModuleHandler = makeSingleton(AddonModThUrlModuleHandlerService);
