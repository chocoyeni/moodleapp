...
import { CoreErrorDebug } from '@classes/errors/error';
import { CoreSiteSchema, CoreSitesProvider } from '@services/sites';
...
export class ThCoreSitesProvider extends CoreSitesProvider {
    ...
    constructor(@Optional() @Inject(CORE_SITE_SCHEMAS) siteSchemas: CoreSiteSchema[][] | null, private device1: Device, private nav1: NavController) {
        super(siteSchemas ?? [], device1, nav1);
    ...
    initialize(): void {
        ...
        CoreEvents.on(CoreEvents.LOGIN, async (data) => {
            if (data.siteId) {
                const site = await thCoreSite.getSite(data.siteId);
        ...
        CoreEvents.on(CoreEvents.SITE_UPDATED, async (data) => {
            if (data.siteId !== thCoreSite.getCurrentSiteId()) {
                return;
            }
    ...
    async visitLink(
        ...
        const site = options.siteId
            ? await thCoreSite.getSite(options.siteId)
            : thCoreSite.getCurrentSite();
    ...
    async logout(options: CoreSitesLogoutOptions = {}): Promise<void> {
        ...
        if (options.removeAccount) {
            await thCoreSite.deleteSite(siteId);
        }
...
export const thCoreSite = makeSingleton(ThCoreSitesProvider);
